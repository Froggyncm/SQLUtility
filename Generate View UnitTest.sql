...
DECLARE @StageSchema				VARCHAR(128),
        ...
        @WarehouseViewSchema        VARCHAR(128) = 'DMload',
        @WarehouseView		        VARCHAR(128) = 'Policy',
...
WHERE v.schema_id = SCHEMA_ID(@WarehouseViewSchema) AND v.name = @WarehouseView AND t.name <> 'sysname';
...
AS
(SELECT DISTINCT 
    'EXEC ' + 'Stage.tSQLt.FakeTableTemporal @SchemaName = ''' + s1.name + ''', @TableName = ''' + t1.name + ''', @Identity = 1, @ComputedColumns = 1, @Defaults = 1, @PreserveNullability = 1, @PreservePrimaryKey = 1, @PreserveTemporal = 1;' SourceTableFakes, 
...
AS
(SELECT DISTINCT 
    'EXEC ' + 'EZLynxWarehouse.tSQLt.FakeTableTemporal @SchemaName = ''' + s1.name + ''', @TableName = ''' + t1.name + ''', @Identity = 1, @ComputedColumns = 1, @Defaults = 1, @PreserveNullability = 1, @PreservePrimaryKey = 1, @PreserveTemporal = 1;' SourceTableFakes, 
    a.DatabaseName, s1.name SourceSchema, t1.name SourceTable, t1.object_id SourceObjectID 
...
WHERE c.name <> 'sysname' AND b.is_column_set = 0
AND fkl.DatabaseName = 'EZLynxWarehouse'),
AllColumnCTE
AS 
(SELECT *
